...
def build_entities() -> list[dict[str, Any]]:
    ...
    change_components = [
        ("CC-01", "Reusable intake record", "Replace repeated capture with one controlled reusable application record."),
        ("CC-02", "Accountable case owner", "Assign one accountable case owner through the complete outcome lifecycle."),
        ("CC-03", "Parallel independent checks", "Run independent checks in parallel after the completeness gate."),
        ("CC-04", "Dedicated exception lane", "Route incomplete, failed or contradictory checks to a controlled exception lane."),
        ("CC-05", "Evidence-bound activation", "Block activation without mandatory evidence and named approval for high risk."),
        ("CC-06", "Automated applicant notifications", "Generate controlled notifications at authorised case states."),
        ("CC-07", "Controlled pilot guardrails", "Apply pilot size, duration, threshold, escalation and halt boundaries."),
        ("CC-08", "Operational observability", "Monitor evidence, queues, exceptions, notification failures and activation controls."),
    ]
    ...
    entities.extend(
        [
            entity("CAP-ONBOARDING", "BUSINESS_CAPABILITY", "Applicant onboarding", "Capability to accept, assess and conclude applicant onboarding.", "BUSINESS_CAPABILITY", ["S-009"]),
            entity("PROC-ONBOARDING", "PROCESS", "Applicant onboarding process", "End-to-end applicant onboarding process.", "END_TO_END_PROCESS", ["S-009", "S-010"]),
            entity("STEP-INTAKE", "PROCESS_STEP", "Capture application", "Capture one reusable application record.", "PROCESS_STEP", ["S-001", "S-010"]),
            entity("STEP-COMPLETENESS", "PROCESS_STEP", "Completeness gate", "Verify mandatory fields and evidence before specialist checks.", "PROCESS_STEP", ["S-010", "S-025"]),
            entity("STEP-DOC-VALIDATION", "PROCESS_STEP", "Document validation", "Validate applicant documents after completeness.", "PROCESS_STEP", ["S-003", "S-010"]),
            entity("STEP-COMPLIANCE", "PROCESS_STEP", "Compliance screening", "Perform compliance screening after completeness.", "PROCESS_STEP", ["S-003", "S-010"]),
            entity("STEP-RISK-REVIEW", "PROCESS_STEP", "Standard-risk review", "Perform standard-risk review after completeness.", "PROCESS_STEP", ["S-003", "S-010"]),
            entity("STEP-EXCEPTION", "PROCESS_STEP", "Exception triage and resolution", "Triage incomplete, failed or contradictory checks.", "PROCESS_STEP", ["S-004", "S-014"]),
            entity("STEP-ACTIVATION-GATE", "PROCESS_STEP", "Evidence-bound activation gate", "Verify evidence and approval before activation.", "PROCESS_STEP", ["S-005", "S-027"]),
            entity("STEP-HIGH-RISK-APPROVAL", "PROCESS_STEP", "High-risk approval", "Capture named human approval for high-risk activation.", "PROCESS_STEP", ["S-005", "S-028"]),
            entity("STEP-NOTIFICATION", "PROCESS_STEP", "Applicant status notification", "Send an approved status notification for authorised states.", "CUSTOMER_AND_COMMUNICATION", ["S-006", "S-033"]),
            entity("PROCEDURE-ONBOARDING", "PROCEDURE", "Controlled onboarding procedure", "Procedure for ownership, checks, exceptions, activation and notifications.", "OPERATING_PROCEDURE", ["S-015"]),
            entity("ROLE-CASE-OWNER", "ROLE", "Case owner", "Accountable owner for case progression and exception coordination.", "ROLE_AND_TEAM", ["S-002", "S-011"]),
            entity("ROLE-DOC-ANALYST", "ROLE", "Document analyst", "Performs document validation.", "ROLE_AND_TEAM", ["S-012"]),
            entity("ROLE-COMPLIANCE-ANALYST", "ROLE", "Compliance analyst", "Performs compliance screening.", "ROLE_AND_TEAM", ["S-012"]),
            entity("ROLE-RISK-ANALYST", "ROLE", "Risk analyst", "Performs standard-risk review and activation evidence review.", "ROLE_AND_TEAM", ["S-012"]),
            entity("ROLE-HIGH-RISK-APPROVER", "ROLE", "High-risk approver", "Named human authority for high-risk activation.", "AUTHORITY_AND_SEGREGATION", ["S-005", "S-028"]),
            entity("TEAM-ONBOARDING", "TEAM", "Onboarding operations team", "Operates onboarding, exception and pilot monitoring activities.", "ROLE_AND_TEAM", ["S-014", "S-048"]),
            entity("AUTH-HIGH-RISK", "AUTHORITY_RULE", "Named high-risk approval authority", "Only the named high-risk approver may confirm a high-risk activation.", "AUTHORITY_AND_SEGREGATION", ["S-028"]),
            entity("SOD-REVIEW-APPROVE", "SEGREGATION_RULE", "Risk review and approval segregation", "The risk reviewer and high-risk approver must be different roles.", "AUTHORITY_AND_SEGREGATION", ["S-029"]),
            entity("TRAIN-CHANGE", "TRAINING_ITEM", "Onboarding change briefing", "Training on ownership, parallel checks, exceptions, activation and notifications.", "SKILL_AND_TRAINING", ["S-013"]),
            entity("SYS-CASE-MANAGEMENT", "SYSTEM", "Case management system", "Stores case, evidence, status and approval records.", "SYSTEM_AND_APPLICATION", ["S-017"]),
            entity("SYS-WORKFLOW", "SYSTEM", "Workflow orchestration system", "Coordinates gates, checks, routing and activation.", "SYSTEM_AND_APPLICATION", ["S-018"]),
            entity("SYS-NOTIFICATION", "SYSTEM", "Notification system", "Sends controlled applicant communications.", "SYSTEM_AND_APPLICATION", ["S-019"]),
            entity("APP-REUSABLE-INTAKE", "APPLICATION_COMPONENT", "Reusable intake component", "Application component that creates and updates the reusable intake record.", "SYSTEM_AND_APPLICATION", ["S-024"]),
            entity("IFACE-SCREENING", "INTERFACE", "Screening interface", "Interface used by workflow to request external screening.", "INTEGRATION_AND_EXTERNAL_SERVICE", ["S-020"]),
            entity("INT-SCREENING", "INTEGRATION", "Screening integration", "Integration between the screening interface and external service.", "INTEGRATION_AND_EXTERNAL_SERVICE", ["S-020"]),
            entity("DATA-APPLICATION", "DATA_OBJECT", "Reusable application record", "Controlled reusable applicant information record.", "DATA_AND_INFORMATION", ["S-001", "S-017"]),
            entity("DATA-EVIDENCE", "DATA_OBJECT", "Evidence pack", "Mandatory evidence associated with the case.", "DATA_AND_INFORMATION", ["S-017", "S-027"]),
            entity("DATA-CASE-STATUS", "DATA_OBJECT", "Case status", "Authorised status used by workflow and notifications.", "DATA_AND_INFORMATION", ["S-017", "S-033"]),
            entity("DATA-APPROVAL", "DATA_OBJECT", "Approval record", "Evidence of named human approval for high-risk activation.", "DATA_AND_INFORMATION", ["S-017", "S-028"]),
            entity("OWNER-ONBOARDING-DATA", "DATA_OWNER", "Onboarding data owner", "Accountable owner of onboarding application, evidence, status and approval data.", "DATA_AND_INFORMATION", ["S-022"]),
            entity("REPORT-PILOT-DASHBOARD", "REPORT", "Pilot operations dashboard", "Reports pilot KPIs and control observations.", "REPORTING_AND_METRICS", ["S-041", "S-048"]),
            entity("KPI-EVIDENCE", "KPI", "Evidence completeness KPI", "Measures evidence completeness at activation.", "REPORTING_AND_METRICS", ["S-042"]),
            entity("KPI-QUEUE", "KPI", "Open queue KPI", "Monitors open pilot cases against the threshold.", "PILOT_AND_OPERATIONAL_GUARDRAIL", ["S-043"]),
            entity("KPI-NOTIFICATION", "KPI", "Notification failure KPI", "Measures controlled notification delivery failures.", "REPORTING_AND_METRICS", ["S-044"]),
            entity("KPI-ACTIVATION", "KPI", "Activation control KPI", "Measures activation attempts against evidence and approval rules.", "CONTROL_AND_POLICY", ["S-045"]),
            entity("SLA-EXCEPTION", "SLA", "Exception handling service target", "Target for exception triage and escalation.", "SERVICE_AND_SUPPORT", ["S-046"]),
            entity("CTRL-COMPLETENESS", "CONTROL", "Completeness control", "Blocks checks until mandatory intake and evidence are present.", "CONTROL_AND_POLICY", ["S-025"]),
            entity("CTRL-INDEPENDENCE", "CONTROL", "Independent checks control", "Permits parallelism only for independent checks.", "CONTROL_AND_POLICY", ["S-026"]),
            entity("CTRL-ACTIVATION", "CONTROL", "Activation evidence control", "Blocks activation without evidence and required approval.", "CONTROL_AND_POLICY", ["S-027"]),
            entity("CTRL-PILOT", "CONTROL", "Pilot boundary and halt control", "Enforces pilot size, duration, threshold and halt conditions.", "PILOT_AND_OPERATIONAL_GUARDRAIL", ["S-031", "S-047"]),
            entity("POLICY-ONBOARDING", "POLICY", "Onboarding evidence and decision policy", "Policy governing completeness, evidence retention, ownership and activation outcomes.", "CONTROL_AND_POLICY", ["S-030"]),
            entity("TOUCH-STATUS", "CUSTOMER_TOUCHPOINT", "Applicant status touchpoint", "Applicant-facing status communications for authorised states.", "CUSTOMER_AND_COMMUNICATION", ["S-033"]),
            entity("COMM-RECEIVED", "COMMUNICATION_TEMPLATE", "Submission received template", "Confirms successful submission and case reference.", "CUSTOMER_AND_COMMUNICATION", ["S-035"]),
            entity("COMM-ACTION", "COMMUNICATION_TEMPLATE", "Action required template", "Requests missing evidence without exposing restricted logic.", "CUSTOMER_AND_COMMUNICATION", ["S-036"]),
            entity("COMM-ACTIVATED", "COMMUNICATION_TEMPLATE", "Activated template", "Confirms activation after evidence and approval gates.", "CUSTOMER_AND_COMMUNICATION", ["S-037"]),
            entity("COMM-DECLINED", "COMMUNICATION_TEMPLATE", "Declined template", "Communicates decline and permitted support route.", "CUSTOMER_AND_COMMUNICATION", ["S-038"]),
            entity("ACCESS-PLAIN-LANGUAGE", "ACCESSIBILITY_REQUIREMENT", "Plain-language requirement", "Notification text must use plain language.", "CUSTOMER_AND_COMMUNICATION", ["S-034", "S-040"]),
            entity("ACCESS-ALTERNATIVE-ROUTE", "ACCESSIBILITY_REQUIREMENT", "Alternative support route", "Notifications must provide an alternative support route.", "CUSTOMER_AND_COMMUNICATION", ["S-034", "S-040"]),
            entity("EXT-SCREENING", "EXTERNAL_SERVICE", "CheckRight screening service", "Fictional external service used for screening.", "INTEGRATION_AND_EXTERNAL_SERVICE", ["S-021", "S-057"]),
            entity("VENDOR-CHECKRIGHT", "VENDOR", "CheckRight", "Fictional supplier of the external screening service.", "INTEGRATION_AND_EXTERNAL_SERVICE", ["S-021", "S-057"]),
            entity("TEST-PARALLEL", "TEST_CASE", "Parallel independent checks test", "Verifies completeness and independence before parallel execution.", "TEST_AND_ASSURANCE", ["S-049"]),
            entity("TEST-ACTIVATION", "TEST_CASE", "Activation gate test", "Verifies evidence and high-risk approval blocking behavior.", "TEST_AND_ASSURANCE", ["S-050"]),
            entity("RUNBOOK-PILOT", "RUNBOOK", "Pilot operations runbook", "Defines monitoring, escalation, halt, recovery and evidence capture.", "PILOT_AND_OPERATIONAL_GUARDRAIL", ["S-051"]),
            entity("SUPPORT-NOTIFICATION", "SUPPORT_PROCEDURE", "Failed notification support procedure", "Defines retry, alternative contact and escalation.", "SERVICE_AND_SUPPORT", ["S-039", "S-052"]),
            entity("ASM-VENDOR-CAPACITY", "ASSUMPTION", "Vendor pilot capacity", "Assumption that the screening vendor can support pilot parallelism.", "INTEGRATION_AND_EXTERNAL_SERVICE", ["S-057"], status="UNRESOLVED"),
            entity("ASM-ACCESSIBILITY", "ASSUMPTION", "Template accessibility validation", "Assumption that templates meet accessibility requirements pending evidence.", "CUSTOMER_AND_COMMUNICATION", ["S-058"], status="UNRESOLVED"),
            entity("ASM-DASHBOARD-LATENCY", "ASSUMPTION", "Dashboard latency", "Assumption that pilot dashboard latency supports operational decisions.", "PILOT_AND_OPERATIONAL_GUARDRAIL", ["S-059"], status="UNRESOLVED"),
            entity("CON-PILOT-LIMIT", "CONSTRAINT", "Ten-percent fourteen-day pilot", "Pilot is limited to ten percent for fourteen calendar days and 300 open cases.", "PILOT_AND_OPERATIONAL_GUARDRAIL", ["S-007", "S-031"]),
            entity("GAP-VENDOR-CAPACITY", "EVIDENCE_GAP", "Vendor capacity evidence gap", "Bounded vendor capacity evidence has not yet been supplied.", "INTEGRATION_AND_EXTERNAL_SERVICE", ["S-057"], status="OPEN"),
            entity("GAP-ACCESSIBILITY", "EVIDENCE_GAP", "Accessibility validation gap", "Template accessibility validation evidence is not yet recorded.", "CUSTOMER_AND_COMMUNICATION", ["S-058"], status="OPEN"),
            entity("GAP-DASHBOARD-LATENCY", "EVIDENCE_GAP", "Dashboard latency evidence gap", "Pilot-load dashboard latency has not yet been measured.", "PILOT_AND_OPERATIONAL_GUARDRAIL", ["S-059"], status="OPEN"),
            entity("NEG-PRICING", "NEGATIVE_CONTROL", "Product pricing", "Product pricing is explicitly unchanged.", "BUSINESS_CAPABILITY", ["S-032", "S-060"], status="EXPLICITLY_UNAFFECTED"),
            entity("NEG-PAYROLL", "NEGATIVE_CONTROL", "Payroll process", "Payroll is explicitly unchanged.", "END_TO_END_PROCESS", ["S-016", "S-060"], status="EXPLICITLY_UNAFFECTED"),
            entity("NEG-CORE-LEDGER", "NEGATIVE_CONTROL", "Core ledger", "The core ledger is explicitly unchanged.", "SYSTEM_AND_APPLICATION", ["S-060"], status="EXPLICITLY_UNAFFECTED"),
            entity("NEG-OFFICE-LOCATION", "NEGATIVE_CONTROL", "Office location", "Office location is explicitly unchanged.", "BUSINESS_CAPABILITY", ["S-016", "S-060"], status="EXPLICITLY_UNAFFECTED"),
            entity("NEG-MARKETING", "NEGATIVE_CONTROL", "Marketing campaigns", "Marketing campaigns are explicitly unchanged.", "CUSTOMER_AND_COMMUNICATION", ["S-060"], status="EXPLICITLY_UNAFFECTED"),
            entity("NEG-EMPLOYEE-BENEFITS", "NEGATIVE_CONTROL", "Employee benefits", "Employee benefits are explicitly unchanged.", "ROLE_AND_TEAM", ["S-060"], status="EXPLICITLY_UNAFFECTED"),
        ]
    )
    return entities
...
def build_relationships() -> list[dict[str, Any]]:
    ...
    for kpi_id, step_id in metric_targets.items():
        add(kpi_id, step_id, "MEASURED_BY", ["S-041", "S-042", "S-043", "S-044", "S-045"], eligible=True, direction="REVERSE")
        add(kpi_id, "REPORT-PILOT-DASHBOARD", "REPORTED_IN", ["S-041"], eligible=True)

    add("TEST-PARALLEL", "STEP-DOC-VALIDATION", "TESTED_BY", ["S-049"], eligible=True, direction="REVERSE")
    add("TEST-PARALLEL", "STEP-COMPLIANCE", "TESTED_BY", ["S-049"], eligible=True, direction="REVERSE")
    add("TEST-PARALLEL", "STEP-RISK-REVIEW", "TESTED_BY", ["S-049"], eligible=True, direction="REVERSE")
    add("TEST-ACTIVATION", "STEP-ACTIVATION-GATE", "TESTED_BY", ["S-050"], eligible=True, direction="REVERSE")
    add("TEST-ACTIVATION", "STEP-HIGH-RISK-APPROVAL", "TESTED_BY", ["S-050"], eligible=True, direction="REVERSE")
    ...
    add("STEP-NOTIFICATION", "SUPPORT-NOTIFICATION", "SUPPORTED_BY", ["S-039", "S-052"], eligible=True)
...
def build_payload(output_root: Path) -> None:
    ...
    write_json(case_dir / "entities.json", {"entities": build_entities(), "schema_version": SCHEMA_VERSION})
    write_json(
...
def main() -> int:
    ...
    if args.check:
        with tempfile.TemporaryDirectory(prefix="bcia-case-") as temporary:
            ...
            actual_paths = [CASE_DIR, EVALUATION_DIR / "answer-key.json", EVALUATION_DIR / "answer-key.sha256", EVALUATION_DIR / "fixtures"]
            actual: dict[str, str] = {}
